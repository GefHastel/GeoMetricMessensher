...
function connectDesktopBridge(tries = 0) {
  ...
  if (typeof QWebChannel === "undefined" || !window.qt || !window.qt.webChannelTransport) {
    if (tries < 40) setTimeout(() => connectDesktopBridge(tries + 1), 50);   // канал ещё не готов — пробуем снова
    return;                                                        // и выходим
  }
  new QWebChannel(window.qt.webChannelTransport, (channel) => {     // создаём канал связи с приложением
    GMDesktop = channel.objects.geometricDesktop;                   // берём «мост» окна
...
function wireDesktopShell() {
  ...
  if (drag && GMDesktop) {                                          // она есть и «мост» доступен —
    drag.addEventListener("mousedown", (e) => {                     // нажали мышью по полосе —
      if (e.button === 0 && GMDesktop.dragStart) GMDesktop.dragStart();   // просим систему начать перетаскивание
    });
    drag.addEventListener("dblclick", () => {                       // двойной щелчок по полосе —
      if (GMDesktop && GMDesktop.dragDoubleClick) GMDesktop.dragDoubleClick();   // развернуть или вернуть окно
    });
  }
  const min = $("win-min"), max = $("win-max"), close = $("win-close");   // три кнопки окна
  if (min) min.addEventListener("click", () => GMDesktop && GMDesktop.winMinimize && GMDesktop.winMinimize());      // свернуть
  if (max) max.addEventListener("click", () => GMDesktop && GMDesktop.winToggleMax && GMDesktop.winToggleMax());    // развернуть
  if (close) close.addEventListener("click", () => GMDesktop && GMDesktop.winClose && GMDesktop.winClose());        // закрыть
...
const socket = API_BASE                                            // адрес есть —
  ? io(API_BASE, { transports: ["polling", "websocket"] })          //   подключаемся (опрос, затем быстрый канал)
...
$("file-input").onchange = async () => {
  ...
  try {
    ...
    const kind = f.type.startsWith("video") ? "video" : (f.type.startsWith("audio") ? "audio" : "image");   // музыка, видео или картинка
...
if (isDesktopShell()) document.documentElement.classList.add("desktop-shell");   // это приложение для компьютера — включаем его оформление
...
function renderPayload(dec) {
  ...
  if (f) {                                                          // если файл есть —
    ...
    } else {                                                        // обычное фото или видео
      out += `<div class="bubble-media" data-file='${esc(JSON.stringify(f))}'><div class="media-loading"></div></div>`;
    }
  }
...
function initMessageDelegation() {
  ...
  box.addEventListener("click", async (e) => {                      // любое нажатие в ленте
    ...
    if (circle) openCircle(circle);                                 //   открываем его на весь экран
  });
}
...
for (const [id, kind] of [["btn-mic", "voice"], ["btn-circle", "circle"]]) {   // кнопка голосового и кнопка кружка
  ...
  const начать = async (e) => {                                     // начали удерживать
    ...
    if (kind === "circle") closeStickerPanel();                    // панели закрываем, чтобы ничего не мешало
